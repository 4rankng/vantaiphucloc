...
# ---------------------------------------------------------------------------
# Terminal Log  (BDST / VIPI style — fixed-position columns)
# ---------------------------------------------------------------------------
...
def extract_terminal_log(rows: list[list]) -> tuple[list[ExtractedRow], list[dict]]:
    ...
    for r, row in enumerate(rows):
        for cell in row:
            if "CONTAINER" in _cell_upper(cell):
                header_idx = r
                break
        if header_idx is not None:
            break
    ...
    for r in range(header_idx + 1, len(rows)):
        ...
        cont_val = _cell_text(row[1]) if len(row) > 1 else ""
        if not cont_val:
            continue
        ...
        shipping_line = _cell_text(row[2]) if len(row) > 2 else ""

        # Parse ISO size -> cont_type (col 3)
        iso_size = _cell_text(row[3]) if len(row) > 3 else ""
        try:
        ...
        # Parse F/E (col 4)
        fe_val = _cell_text(row[4]) if len(row) > 4 else "E"
        try:
        ...
        work_type_raw = _cell_text(row[6]) if len(row) > 6 else ""
