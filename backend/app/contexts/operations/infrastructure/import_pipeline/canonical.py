...
CANONICAL_FIELDS: tuple[CanonicalField, ...] = (
    ...
    CanonicalField(
        "gross_weight_kg", "Trọng lượng (kg)", False, "VGM hoặc gross weight."
    ),
    ...
    CanonicalField(
        "trip_date", "Ngày chuyến", False, "Khi không có ngày đi/đến riêng."
    ),
    CanonicalField(
        "customer_ref",
        "Số booking / B/L",
        False,
        "Hoặc số work order, dùng để tra trùng.",
    ),
    ...
    CanonicalField(
        "work_type", "Tác nghiệp", False, "Loại tác nghiệp / operation type."
    ),
...
SYNONYMS: dict[str, list[str]] = {
    "container_no": [
        ...
        "số cont",
        "socontainer",
        "sốcontainer",  # no-space variant from SL sheets
    ...
    "container_type_iso": [
        ...
        "container type",
        "kiech iso",
        "kiechiso",  # BDST/VIPI compact: kích cỡ ISO
    ],
    "freight_kind": [
        "f/e",
        "fe",
        ...
        "empty",
        "空/重",
        "空/重(e/f)",  # Chinese: empty/heavy (E/F)
    ],
    "gross_weight_kg": [
        "weight",
        "vgm",
    ...
    "trip_date": [
        ...
        "thoi gian xuat hien",
        "thời gian xuất hiện",
        "thoi gian tiep can xe",
        "thời gian tiếp cận xe",
    ],
    "customer_ref": [
        "booking",
        "booking no",
...
def _build_skip_lookup() -> frozenset[str]:
    return frozenset(
        normalize_header_text(p) for p in SKIP_PATTERNS if normalize_header_text(p)
    )
