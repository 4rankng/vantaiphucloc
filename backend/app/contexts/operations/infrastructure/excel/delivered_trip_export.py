...
async def generate_delivered_trips_excel(
    ...
    if date_from:
        query = query.where(
            DeliveredTrip.trip_date >= date_type.fromisoformat(date_from)
        )
    if date_to:
    ...
    loc_ids = {wo.pickup_location_id for wo in delivered_trips} | {
        wo.dropoff_location_id for wo in delivered_trips
    }
