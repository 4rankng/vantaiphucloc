...
async def generate_vendor_export_excel(
    ...
    ws.append(
        [
            f"BẢNG CHUYẾN – {vendor.name.upper()} – {month_range}",
            *([""] * (num_cols - 1)),
        ]
    )
    ...
    header_fill = PatternFill(
        start_color=_blue_header, end_color=_blue_header, fill_type="solid"
    )
    ...
    for row_idx, wo in enumerate(delivered_trips):
        ...
        stt += 1
        cont_label = WORK_TYPE_FULL.get(
            wo.cont_type or "", wo.cont_type or wo.work_type or ""
        )
    ...
    sum_fill = PatternFill(
        start_color=_blue_light, end_color=_blue_light, fill_type="solid"
    )
    ...
    total_fill = PatternFill(
        start_color=_yellow_sum, end_color=_yellow_sum, fill_type="solid"
    )
