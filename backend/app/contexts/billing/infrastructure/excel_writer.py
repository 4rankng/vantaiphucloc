...
def _write_sl_sheet(
    wb: openpyxl.Workbook, data: SettlementStatement, sheet_name: str
) -> None:
    ...
    if data.trip_lines:
        ...
        sum_cell = ws.cell(
            row=total_row, column=14, value=f"=SUM(N{first_data_row}:N{last_data_row})"
        )
        _money_fmt(sum_cell)
        sum_cell.font = _BOLD
        ...
        grand_cell = ws.cell(
            row=grand_row, column=14, value=f"=N{total_row}+N{vat_row}"
        )
...
def _write_bktt_sheet(
    ...
    ws.merge_cells(
        start_row=header_top, start_column=2, end_row=header_top, end_column=3
    )
    ws.merge_cells(
        start_row=header_top, start_column=5, end_row=header_top, end_column=8
    )
    ...
    for stt, summary in enumerate(data.route_summary, start=1):
        ...
        ws.cell(
            row=row, column=7, value=summary.empty_count or None
        ).alignment = _CENTER
        ws.cell(row=row, column=8, value=None).alignment = _CENTER
        ws.cell(
            row=row, column=9, value=f"=E{row}+F{row}+G{row}+H{row}"
        ).alignment = _CENTER
    ...
    if data.route_summary:
        ...
        ws.cell(
            row=total_row,
            column=9,
            value=f"=E{total_row}+F{total_row}+G{total_row}+H{total_row}",
        ).font = _BOLD
        ...
        ws.merge_cells(
            start_row=vat_row, start_column=1, end_row=vat_row, end_column=10
        )
        ...
        ws.merge_cells(
            start_row=grand_row, start_column=1, end_row=grand_row, end_column=10
        )
        grand_cell = ws.cell(
            row=grand_row, column=11, value=f"=K{total_row}+K{vat_row}"
        )
        _money_fmt(grand_cell)
        grand_cell.font = _BOLD
        grand_cell.fill = _TOTAL_FILL
        ...
        ws.merge_cells(
            start_row=words_row, start_column=3, end_row=words_row, end_column=12
        )
        ...
        c.alignment = _CENTER
        ws.merge_cells(
            start_row=sign_row, start_column=10, end_row=sign_row, end_column=12
        )
        ...
        ws.merge_cells(
            start_row=sign_label_row,
            start_column=2,
            end_row=sign_label_row,
            end_column=4,
        )
...
def settlement_filename(data: SettlementStatement) -> str:
    code = (
        (data.client.code or data.client.name or "KH").strip().upper().replace(" ", "_")
    )
    mm = data.period.end.month
    yy = data.period.end.year % 100
