...
async def _load_alias_or_404(alias_id: int, db: AsyncSession):
    ...
    if orm is None:
        raise HTTPException(
            status_code=404, detail=f"LocationAlias {alias_id} not found"
        )
    return orm
...
async def list_aliases(
    ...
    q = select(LocationAliasORM, LocationORM.name.label("location_name")).join(
        LocationORM, LocationORM.id == LocationAliasORM.location_id
    )
    if location_id:
    ...
    return [
        _alias_to_out(alias_orm, location_name=loc_name) for alias_orm, loc_name in rows
    ]
...
async def merge_locations(
    ...
    if body.source_location_id == body.target_location_id:
        raise HTTPException(
            status_code=400, detail="Cannot merge a location into itself"
        )
    ...
    if source_orm is None:
        raise HTTPException(
            status_code=404,
            detail=f"Source location {body.source_location_id} not found",
        )
    if target_orm is None:
        raise HTTPException(
            status_code=404,
            detail=f"Target location {body.target_location_id} not found",
        )
    ...
    if source_name_norm and source_name_norm != target_name_norm:
        ...
        existing = (
            await db.execute(
                select(LocationAliasORM).where(
                    LocationAliasORM.alias_normalized == source_name_norm
                )
            )
        ).scalar_one_or_none()
        if existing is None:
