...
async def resolve_driver_plate(session: AsyncSession, driver_id: int) -> str | None:
...
async def ensure_vehicle(session: AsyncSession, plate: str) -> Vehicle:
...
async def deactivate_existing_assignments(
    ...
    for vd in existing:
        ...
        if vd.vehicle_id is not None:
            await session.execute(
                update(Vehicle)
                .where(Vehicle.id == vd.vehicle_id, Vehicle.driver_id == driver_id)
