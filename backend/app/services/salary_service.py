...
async def get_salary_period_dates(
    ...
    if from_day <= to_day:
        return date(year, month, from_day), date(year, month, to_day)

    # Period crosses a month boundary (e.g. 26th → 25th).
    if reference_date.day >= from_day:
        start = date(year, month, from_day)
        next_month = month + 1 if month < 12 else 1
        next_year = year if month < 12 else year + 1
        end = date(next_year, next_month, to_day)
    else:
        prev_month = month - 1 if month > 1 else 12
        prev_year = year if month > 1 else year - 1
        start = date(prev_year, prev_month, from_day)
        end = date(year, month, to_day)
