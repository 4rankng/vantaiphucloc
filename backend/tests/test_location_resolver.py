...
async def test_resolver_exact_alias_match(db_session):
    ...
    db_session.add(
        LocationAlias(
            location_id=loc.id,
            alias="TCCL",
            alias_normalized=normalize("TCCL"),
            source="seed",
        )
    )
    await db_session.flush()
...
async def test_resolver_resolve_or_create_idempotent(db_session):
    ...
    a = await r.resolve_or_create(
        "Brand New Yard 123", source=ResolverSource.IMPORT, user_id=None
    )
    ...
    cnt = await db_session.execute(
        ...
        .where(Location.name == "Brand New Yard 123")
    )
