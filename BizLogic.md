...
- **Operations** — extracted to `backend/app/contexts/operations/` (commit 2026-05-05). Domain + infrastructure are done; application + interface layers are deferred to a follow-up. State:
  ...
  - **Application + interface**: empty placeholders. The legacy paths still serve all `/trip-orders`, `/work-orders`, `/reconcile`, and `/imports` endpoints — `app/api/v1/{trip_orders,work_orders,reconcile,imports}.py`, `app/services/{trip_order_service,work_order_service,matching_service,state_machine,import_pipeline}.py`, and `app/repositories/{trip_order_repo,work_order_repo}.py` are unchanged.
  - **Tests**: `backend/tests/contexts/operations/test_domain.py` covers state transitions, lock semantics, container rules, and the apply-pricing-snapshot invariant.

The remaining 5 contexts (Fleet, Imports, Billing, Payroll, plus the cross-cutting AuditLog) still live in the legacy folder layout.
