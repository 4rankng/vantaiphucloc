...
/**
 * Agent state type compatible with AgentAudioVisualizerAura.
 * We define our own instead of importing from @livekit/components-react
 * since this project doesn't use LiveKit.
 */
...
export function useAgentAudioVisualizerAura(
  ...
  // Simulate subtle audio-driven scale pulses when speaking
  useEffect(() => {
    if (state === 'speaking' && !scaleMotionValue.isAnimating()) {
      animateScale(0.35, { duration: 0.3, ease: 'easeOut' });
      const id = setTimeout(() => {
        animateScale(0.25, { duration: 0.3, ease: 'easeIn' });
      }, 300);
