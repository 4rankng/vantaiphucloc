...
import { InfoTip } from '@/components/shared/feedback/InfoTip'
          ...
          <div className={renderMobileCard ? 'hidden lg:block' : ''}>
            {data.map((row, i) => (
              <div
                ...
                className={`entity-row-enter lg:grid gap-4 px-5 py-3 transition-colors hover:bg-[var(--theme-bg-tertiary)] ${
            ...
            <div className="flex flex-col divide-y lg:hidden" style={{ borderColor: 'var(--theme-border-light)' }}>
              {data.map((row, i) => (
                <div
                  key={rowKey(row)}
                  onClick={() => onRowClick?.(row)}
                  className={`entity-row-enter p-4 transition-colors hover:bg-[var(--theme-bg-tertiary)] ${onRowClick ? 'cursor-pointer' : ''}`}
                  style={{
                    animationDelay: `${Math.min(i * 28, 280)}ms`,
                  }}
                >
