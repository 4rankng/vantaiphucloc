  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        ...
        {onCreateClick && (
          <Button onClick={onCreateClick} className="shrink-0 gap-1.5 bg-gold-400 text-navy-950 hover:bg-gold-300 font-semibold">
      ...
      {showFilters && filters.length > 0 && (
        <div className="flex flex-wrap gap-2 p-3 glass-card rounded-lg animate-fade-slide-up">
          ...
          <Button variant="ghost" size="sm" className="text-xs text-gray-400">Đặt lại</Button>
        </div>
      )}
    </div>
  )
...
export function EmptyState({ icon, title, description }: EmptyStateProps) {
  return (
    <div className="py-16 text-center">
      <div className="w-16 h-16 rounded-full bg-navy-50 flex items-center justify-center mx-auto mb-4 text-navy-300">
        {icon}
      </div>
...
export function MobileListCard({ children, onClick, className }: MobileListCardProps) {
  return (
    <div
      onClick={onClick}
      className={`glass-card rounded-xl p-4 animate-fade-slide-up ${onClick ? 'cursor-pointer hover:shadow-md transition-shadow active:scale-[0.99]' : ''} ${className || ''}`}
    >
      {children}
    </div>
  )
