...
type PageTab = 'orders' | 'clients' | 'reports'
      ...
      {TABS.map(tab => {
        ...
        return (
          <button
            ...
            className="flex items-center gap-1.5 flex-1 justify-center py-1.5 px-3 rounded-lg text-sm font-semibold transition-all"
            style={{
...
export function TripList() {
  ...
  // Director uses direct navigation (no tabs, no import dialog)
  if (isDirector) {
  ...
  if (!isMobile) {
    return (
      <div className="space-y-5">
        ...
        {/* ── Tab: Khách hàng ── */}
        {activeTab === 'clients' && <ClientsAndVendors />}

        {/* ── Tab: Báo cáo ── */}
        {activeTab === 'reports' && <CustomerSettlementReport />}
...
// ─── Director-only view (no tabs, no import) ──────────────────────────────────
