...
export function TripDetailCard({ workOrder, onEdited }: TripDetailCardProps) {
  ...
  if (!onEdited) {
    return <TripDetailCardView workOrder={workOrder} onEdited={onEdited} />
  }

  return <TripDetailCardEditable workOrder={workOrder} onEdited={onEdited} />
}

export { TripDetailCard as default }

function TripDetailCardView({ workOrder, onEdited }: { workOrder: WorkOrder; onEdited?: () => void }) {
  const plate = workOrder.driver?.vehicle?.plate ?? workOrder.vehicleExternalPlate
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {plate ? (
          ...
          <span className="text-xs font-semibold" style={{ color: 'var(--theme-brand-primary)' }}>
            {workOrder.driver?.name || 'Xe ngoài'}
          </span>
        )}
