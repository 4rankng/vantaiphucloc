...
import { MapPin, Plus, AlertTriangle, Merge, ArrowUp, Trash2, Search, X, Check, Pencil } from 'lucide-react'
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <header className="px-6 py-5 shrink-0" style={{ borderBottom: '1px solid var(--line)' }}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-[11px] uppercase tracking-wide mb-1" style={{ color: 'var(--ink-4)' }}>
              Địa điểm
            </p>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {duplicate && (
              ...
              >
                <Merge className="h-4 w-4" /> Gộp vào {duplicate.name}
              </Button>
            )}
            <button
              type="button"
              ...
              style={{ width: 32, height: 32, color: 'var(--ink-3)' }}
              title="Xoá địa điểm"
            >
        ...
        {duplicate && (
          ...
          >
            <AlertTriangle className="h-3.5 w-3.5 shrink-0 mt-0.5" style={{ color: 'var(--warning)' }} />
            <p className="text-[12px] m-0" style={{ color: 'var(--ink-2)' }}>
              Có thể trùng với <strong>{duplicate.name}</strong>. Nếu cùng một địa điểm, hãy gộp lại để
              tránh trùng dữ liệu.
            </p>
          </div>
        )}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        <h3
          className="text-[11px] uppercase tracking-wide m-0 mb-3"
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {adding ? (
            ...
            >
              ...
              {aliases.length === 0 ? 'Thêm tên phụ' : 'Thêm'}
            </button>
          )}
        </div>

        {aliases.length === 0 && !adding && (
          <p className="text-[12px] m-0 mt-3" style={{ color: 'var(--ink-3)' }}>
      ...
      {/* Metadata footer */}
      <footer
        className="px-6 py-3 shrink-0 flex items-center gap-4 text-[11px]"
        style={{ borderTop: '1px solid var(--line)', color: 'var(--ink-3)' }}
      >
        <span>
          Tạo: {new Date(location.createdAt).toLocaleDateString('vi-VN')}
        </span>
        {location.updatedAt && location.updatedAt !== location.createdAt && (
          <span>
            Cập nhật: {new Date(location.updatedAt).toLocaleDateString('vi-VN')}
          </span>
        )}
        <span className="ml-auto tabular-nums" style={{ color: 'var(--ink-4)' }}>
          #{location.id}
        </span>
      </footer>
    </div>
