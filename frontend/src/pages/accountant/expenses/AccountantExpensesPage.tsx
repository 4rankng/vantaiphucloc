            ...
            <MobileListCard key={e.id} onClick={() => setDetailId(e.id)}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded bg-gold-50 flex items-center justify-center text-gold-500"><CircleDollarSign size={14}/></div>
                  <span className="text-xs font-bold text-navy-900">{e.category}</span>
                </div>
                <StatusBadge variant={expStatusVariant(e.status)} label={expStatusLabel(e.status)} />
              </div>
              <p className="text-[12px] text-navy-900">{e.description}</p>
              <div className="flex items-center justify-between mt-2 text-[11px]">
                <span className="text-gray-400">{e.tractorPlate} · {e.date}</span>
                <span className="font-bold text-navy-900 font-mono-num">{formatCurrencyShort(e.amount)}</span>
              </div>
              {e.status === 'DRAFT' && (
        ...
        <GlassCard className="overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>{mockExpenses.map((e) => (
              <tr key={e.id} className="border-b border-navy-50 last:border-0 hover:bg-navy-50/30 cursor-pointer" onClick={() => setDetailId(e.id)}>
                <td className="px-4 py-2.5 font-semibold text-navy-900 font-mono-num">{e.id}</td>
                <td className="px-4 py-2.5 text-navy-900">{e.category}</td>
                <td className="px-4 py-2.5 text-gray-500 max-w-[200px] truncate">{e.description}</td>
                <td className="px-4 py-2.5 text-gray-500 font-mono-num">{e.tractorPlate}</td>
                <td className="px-4 py-2.5 text-gray-500 font-mono-num">{e.date}</td>
                <td className="px-4 py-2.5"><StatusBadge variant={expStatusVariant(e.status)} label={expStatusLabel(e.status)} /></td>
                <td className="px-4 py-2.5 text-right font-semibold text-navy-900 font-mono-num">{formatCurrencyShort(e.amount)}</td>
      ...
      <DetailModal open={!!detail} onOpenChange={() => setDetailId(null)} title="Chi tiết chi phí">
        {detail && (
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between py-2 border-b border-navy-100"><span className="text-gray-500">Danh mục</span><span className="text-navy-900">{detail.category}</span></div>
            ...
            <div className="flex justify-between py-2 border-b border-navy-100"><span className="text-gray-500">Xe</span><span className="text-navy-900 font-mono-num">{detail.tractorPlate}</span></div>
            <div className="flex justify-between py-2 border-b border-navy-100"><span className="text-gray-500">Ngày</span><span className="text-navy-900">{detail.date}</span></div>
            ...
            <div className="flex justify-between py-2"><span className="text-gray-500">Trạng thái</span><StatusBadge variant={expStatusVariant(detail.status)} label={expStatusLabel(detail.status)} /></div>
          </div>
        )}
      </DetailModal>
