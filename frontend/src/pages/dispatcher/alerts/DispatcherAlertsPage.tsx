              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${a.severity === 'high' ? 'bg-red-100 text-red-700' : a.severity === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'}`}>
                      {severityLabel(a.severity)}
                    </span>
                    <span className="text-[10px] text-gray-400">{a.timestamp}</span>
                  </div>
                  <p className="text-[12px] text-navy-900">{a.message}</p>
                </div>
              </div>
            </MobileListCard>
          ))}
        ...
        <GlassCard className="overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>{mockAlerts.map((a) => (
              <tr key={a.id} className={`border-b border-navy-50 last:border-0 ${a.severity === 'high' ? 'bg-red-50/20' : ''}`}>
                <td className="px-4 py-3"><span className={`text-[10px] font-semibold px-2 py-1 rounded-full ${a.severity === 'high' ? 'bg-red-100 text-red-700' : a.severity === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'}`}>{severityLabel(a.severity)}</span></td>
                <td className="px-4 py-3">{typeIcon(a.type)}</td>
                <td className="px-4 py-3 text-navy-900">{a.message}</td>
                <td className="px-4 py-3 text-gray-500 text-[12px]">{a.timestamp}</td>
              </tr>
            ))}</tbody>
          </table>
        </GlassCard>
