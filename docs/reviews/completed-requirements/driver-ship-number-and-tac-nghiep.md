...
2. **Tác nghiệp (operation type)**
   ...
   - Working enum (confirmed by Huyền 2026-05-13): `XUAT_TAU` (Xuất tàu), `NHAP_TAU` (Nhập tàu), `CHUYEN_BAI` (Chuyển bãi). Confirm whether the list is complete before locking.
   - **Tác nghiệp also drives pricing** — see [pricing-shipper-and-operation.md](./pricing-shipper-and-operation.md). Same route in reverse direction is often a different tác nghiệp at a different rate.
...
- Is the tác nghiệp list (XUAT_TAU / NHAP_TAU / CHUYEN_BAI) complete, or are there more values?
