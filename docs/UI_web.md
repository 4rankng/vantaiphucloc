...
Fixed at top on every page. Contains global actions.

```
┌─────────────────────────────────────────────────┐
│ 🪐 TTransport              🔔 (3)  👤 An       │  Top Bar
├─────────────────────────────────────────────────┤
│ [Content Area]                                  │
├──────────┬──────────┬──────────┬────────────────┤
...
|----------|------|---------|----------|
| Left | 🪐 Logo | Brand identity | Tap → role dashboard |
| Right-1 | 🔔 Notifications | Unread count badge | Tap → Notifications page |
| Right-2 | 👤 Avatar | Account & profile | Tap → Profile page |

**Rules:**
- 🔔 red badge with unread count (max 99+)
- 👤 avatar or initials circle
- On sub-pages: back arrow replaces logo, 🔔 + 👤 stay on right
- On desktop: top bar is part of sidebar header, not floating

| Prop | Type | Description |
|------|------|-------------|
| `unread_count` | `number` | Badge on 🔔 |
| `user_name` | `string` | Display name/initials |
| `avatar_url` | `string?` | Photo or initials |
...
Sidebar includes top bar actions (🔔 + 👤) in its header area.

```
┌─────────────────┬──────────────────────────┐
│ 🪐 TTransport    │                          │
│ 🔔 (3)   👤 An ▸ │  Top bar (in sidebar)    │
├─────────────────┤     [Content Area]        │
│ 📊 Tổng quan     │                          │
│                 │                          │
│ 🚛 Vận tải  ▸   │                          │
...
Notifications (🔔) and Profile (👤) are in the **top bar** — not here.
...
```
┌─────────────────────────────────┐
│ 🪐 Logo    🔔 (3)   👤 An      │  TopBar (fixed, all pages)
├─────────────────────────────────┤
│ [Header] — title, back, actions │  Page header (optional, sub-pages)
├─────────────────────────────────┤
│                                 │
│        [Scrollable Content]     │
│                                 │
├──────┬──────┬──────┬───────────┤
│ Nav  │ Nav  │ Nav  │ Nav       │  BottomNav (mobile only)
└──────┴──────┴──────┴───────────┘
```
...
| `actions` | `ReactNode` | Page-specific header actions |
| `scroll` | `boolean` | Content scrollable (default true) |
| `hideTopBar` | `boolean` | Hide top bar (e.g. full-screen map) |
